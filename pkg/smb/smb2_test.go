...
var fs *smb.Share
var rfs *smb.Share
var session *smb.Session
var dialer *smb.Dialer
...
func TestContextError(t *testing.T) {
	...
	checkError1 := func(op string, err error) {
		if err == nil || err.(*smb.ContextError).Err != context.Canceled {
	...
	checkError2 := func(op string, err error) {
		switch e := err.(type) {
		case *os.PathError:
			err = e.Err.(*smb.ContextError).Err
